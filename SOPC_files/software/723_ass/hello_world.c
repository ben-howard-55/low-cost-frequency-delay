...
struct LoadStatus {
	uint_32 activatedLoads;
	uint_32 blockedLoads;
};
...
void setupTasks() {
	xTaskCreate(maintenanceTask, "Maintenance Task", configMINIMAL_STACK_SIZE, NULL, MAINTENANCE_TASK_PRIORITY, NULL);
	xTaskCreate(frequencyAnalyserTask, "Frequency Analyser Task",configMINIMAL_STACK_SIZE, NULL, FREQUENCY_TASK_PRIORITY, NULL);
	xTaskCreate(loadManagerTask, "Load Manager Task", configMINIMAL_STACK_SIZE, NULL, LOAD_MANAGER_TASK_PRIORITY, NULL);
	xTaskCreate(keyboardTask, "Keyboard Task", configMINIMAL_STACK_SIZE, NULL, KEYBOARD_TASK_PRIORITY, NULL);
	xTaskCreate(vgaRefreshTask, "VGA Display Task", configMINIMAL_STACK_SIZE, NULL, VGA_DISPLAY_TASK_PRIORITY, NULL);
	xTaskCreate(ledManagerTask, "LED Manager Task", configMINIMAL_STACK_SIZE, NULL, LED_MANAGER_TASK_PRIORITY, NULL);
	xTaskCreate(switchPollTask, "Switch Monitor Task", configMINIMAL_STACK_SIZE, NULL, SWITCH_MONITOR_TASK_PRIORITY, NULL);
}
...
void setupTimers() {
	loadManagementTimer = xTimerCreate("Load Management Timer", pdMS_TO_TICKS(500), pdFALSE, &loadManagementTimerId, loadManagerTask);
	vgaRefreshTimer = xTimerCreate("load Timer", pdMS_TO_TICKS(1000), pdFALSE, &vgaRefreshTimerId, vgaRefreshTask);
	switchPollTimer = xTimerCreate("load Timer", pdMS_TO_TICKS(1000), pdFALSE, &switchPollTimerId, switchPollTask);
}
...
static void frequencyAnalyserTask(void *pvParameters) {
	...
	while(1) {
			...
			while (uxQueueMessagesWaiting( frequencyQueue ) != 0) {
				...
				xQueueReceive( frequencyQueue, frequencyHistoryState.frequencyHistory + i, 0 );
				*(frequencyHistoryState.frequencyHistory + i) = SAMPLING_FREQUENCY / (*(frequencyHistoryState.frequencyHistory - 1));

				int signalFrequency = SAMPLING_FREQUENCY / (*(frequencyHistoryState.frequencyHistory - 1));
				int frequencyRoc =
...
static void switchPollTask(void *pvParameters) {
	while (1) {
		...
		if (!loadManagement.isManagingLoads) {
			...
			loads.activatedLoads = activatedLoadsState.activatedLoads - blockedLoadsState.blockedLoads;
